...
def test_same_shell_and_live_prompt_preserve_partial_input(ssh_fixture) -> None:
    ...
    redraw_start = shell.output_position()
    shell.wait_for_prompt_text("sync ", timeout=5.0, start=redraw_start)
    shell.wait_for_prompt_change(timeout=5.0, start=redraw_start)
    terminal = shell.terminal_state()

    assert shell.first_sync_status_at - started < 1.0
    assert terminal.remote_shell_pid == original_pid
    assert terminal.visible_input == "python tra"
    assert terminal.cursor_offset == len("python tra")
